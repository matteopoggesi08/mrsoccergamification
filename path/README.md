...
- Rosa giocatori + staff tecnico con archiviazione/cestino
- Sedute allenamento/partita
- Presenze giocatori + staff tecnico + motivi assenza
...
## V10.1 FIX
- Staff RLS policies use the existing authorization functions `is_member` and `can_do`; no undefined `can_access_season` / `can_edit_season` functions are required.
- Home avoids Supabase session calls when environment variables are not configured.
- Added a runtime error boundary so frontend exceptions are shown instead of a blank screen.

## V11 - Staff in session + player document data
- Staff tecnico presente direttamente nella schermata della seduta.
- Presenze staff salvate in `staff_attendance`, separate dalle statistiche giocatori.
- Aggiunti ai giocatori: tipo documento, numero documento e data di nascita.
- PDF convocazione arricchito con i dati anagrafici/documentali dei convocati.
- UI presenze resa più leggibile con pulsanti presenza/assenza più grandi e colonne dimensionate correttamente.
