...
-- =========================================================
-- TECHNICAL STAFF (separate from players)
-- =========================================================
do $$begin
  if not exists (select 1 from pg_type where typname='staff_role' and typnamespace='public'::regnamespace) then
    create type public.staff_role as enum(
      'HEAD_COACH','ASSISTANT_COACH','FITNESS_COACH','GOALKEEPER_COACH',
      'TECHNICAL_COLLABORATOR','TEAM_MANAGER','OTHER'
    );
  end if;
end$$;
...
create table if not exists public.technical_staff(
  ...
  role public.staff_role not null default 'OTHER',
  phone text,
  notes text,
...
create index if not exists idx_technical_staff_season on public.technical_staff(season_id,status);
create index if not exists idx_staff_attendance_session on public.staff_attendance(session_id);
...
drop policy if exists technical_staff_select on public.technical_staff;
drop policy if exists technical_staff_write on public.technical_staff;
...
create policy technical_staff_select on public.technical_staff
for select using (public.can_access_season(season_id));

create policy technical_staff_write on public.technical_staff
for all using (public.can_edit_season(season_id)) with check (public.can_edit_season(season_id));

create policy staff_attendance_select on public.staff_attendance
for select using (
  exists(select 1 from public.sessions s where s.id=session_id and public.can_access_season(s.season_id))
);

create policy staff_attendance_write on public.staff_attendance
for all using (
  exists(select 1 from public.sessions s where s.id=session_id and public.can_edit_season(s.season_id))
) with check (
  exists(select 1 from public.sessions s where s.id=session_id and public.can_edit_season(s.season_id))
);
