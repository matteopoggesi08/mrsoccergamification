...
for select using (
  exists(
    ...
    where s.id = session_id
      and public.is_member(s.season_id)
  )
);
...
for all using (
  exists(
    select 1 from public.sessions s
    where s.id = session_id
      and public.can_do(s.season_id,'attendance')
  )
)
with check (
  exists(
...
-- ============================================================
-- V11 PLAYER DOCUMENT DATA
-- Safe migration: adds only missing columns, preserves data.
-- ============================================================
