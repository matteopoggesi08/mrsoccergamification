...
type Side = 'A' | 'B' | 'NONE';
type MatchPlayer = { player_id: string; team_side: Side; minutes_played: number };
type Match = {
  id: string;
  team_a_name: string;
  team_b_name: string;
...
// Ciclo di assegnazione: non assegnato → Squadra A → Squadra B → Non ha
// giocato → non assegnato. "Non ha giocato" è uno stato esplicito e
// diverso da "non ancora assegnato": permette di segnare chi era
// presente ma non ha preso parte a questa partitella, senza lasciarlo
// semplicemente non gestito.
function nextSide(current: Side | null): Side | null {
  if (current === null) return 'A';
  if (current === 'A') return 'B';
  if (current === 'B') return 'NONE';
  return null;
}
...
export function MatchCard({
  ...
  const playedAssignments = assignments.filter((mp) => mp.team_side !== 'NONE');
  ...
  return (
    <div className="space-y-3 rounded-xl border bg-card p-4">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                side === 'NONE' && 'border-muted-foreground/30 bg-muted text-muted-foreground line-through',
      ...
      <p className="text-xs text-muted-foreground">
        Tocca un giocatore per assegnarlo: non assegnato → squadra A → squadra B → non ha giocato
      </p>

      {playedAssignments.length > 0 && (
        <div className="space-y-1.5 border-t pt-2">
          <p className="text-xs font-medium text-muted-foreground">Minuti giocati in questa partita</p>
          {playedAssignments.map((mp) => {
