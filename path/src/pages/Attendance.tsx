...
export default function Attendance() {
  ...
  return <Page season={season} title="Presenze" sub="Storico presenze e promemoria delle indisponibilità programmate" actions={<button className="primary" onClick={() => openPlanned()}><Plus />Assenza programmata</button>}>
    ...
    {tab !== 'register' && tab !== 'player' && <section>
      <div className="section-bar"><div><h2>Assenze programmate</h2><p>Promemoria: non modifica le presenze delle sedute. Le assenze reali si registrano sempre nella seduta.</p></div></div>
      <div className="filters"><label className="filter-label">Data di riferimento<input type="date" value={plannedDate} onChange={e => setPlannedDate(e.target.value)} /></label></div>
      <div className="stats">
      ...
      <div className="section-bar"><div><h3>Calendario disponibilità · prossimi 14 giorni</h3><p>Stima basata esclusivamente sulle assenze che hai già programmato.</p></div></div><div className="cards">{availabilityCalendar.map(x => <div className="info" key={x.date}><div><b>{date(x.date)}</b><small>{x.unavailable ? `${x.unavailable} indisponibili programmati` : 'Nessuna indisponibilità programmata'}</small></div><strong>{x.available} presenti previsti</strong></div>)}</div>{upcoming.length ? <div className="cards">{upcoming.map(a => { const p = players.find(x => x.id === a.player_id); return <div className="info" key={a.id}><div><b>{p ? `${p.first_name} ${p.last_name}` : 'Giocatore'}</b><small>{date(a.start_date)} → {date(a.end_date)}{a.reason ? ` · ${a.reason}` : ''}{a.notes ? ` · ${a.notes}` : ''}</small></div><div className="row-actions"><button className="small" onClick={() => openPlanned(a)}><Edit3 size={14} />Modifica</button><button className="icon danger-icon" onClick={() => setConfirm(a)}><Trash2 size={16} /></button></div></div> })}</div> : <Empty title="Nessuna assenza programmata" text="Aggiungi qui le indisponibilità che i giocatori ti hanno comunicato in anticipo." />}
    </section>}
    {tab !== 'planned' && <section><div className="section-bar"><div><h2>Riepilogo presenze</h2><p>Le presenze effettive vengono registrate direttamente nelle sedute.</p></div></div><div className="filters"><div className="search"><Search size={17}/><input placeholder="Cerca giocatore…" value={q} onChange={e => setQ(e.target.value)} /></div><select value={player} onChange={e => setPlayer(e.target.value)}><option value="ALL">Tutti i giocatori</option>{players.map(p => <option key={p.id} value={p.id}>{p.first_name} {p.last_name}</option>)}</select><select value={type} onChange={e => setType(e.target.value)}><option value="ALL">Tutte le sedute</option><option value="TRAINING">Allenamenti</option><option value="MATCH">Partite</option></select><input type="date" value={from} onChange={e => setFrom(e.target.value)}/><input type="date" value={to} onChange={e => setTo(e.target.value)}/></div>
      {tab === 'overview' && (rows.length ? <div className="table-card"><div className="table-head three"><span>Giocatore</span><span>Presenze</span><span>%</span></div>{rows.map(r => <div className="table-row three" key={r.id}><b>{r.first_name} {r.last_name}</b><span>{r.present}/{r.total}</span><strong>{r.percentage}%</strong></div>)}</div> : <Empty title="Nessun dato" text="Crea sedute e registra le presenze."/>}
      {tab === 'register' && <><div className="section-bar"><div><h3>Registro giorno per giorno</h3><p>Vista cronologica delle presenze effettive, comprese le partite tramite convocazione.</p></div></div>{filteredSessions.length ? <div className="cards">{filteredSessions.slice(0,60).map(s => { const recMap = new Map(records.filter(r => r.session_id === s.id).map(r => [r.player_id, r])); const matchId = matchIdsForSession.get(s.id); const callMap = new Map(calls.filter(c => c.match_id === matchId).map(c => [c.player_id, c])); const relevant = (player === 'ALL' ? filteredPlayers : filteredPlayers.filter(p => p.id === player)); return <div className="info attendance-day" key={s.id}><div><b>{date(s.session_date)} · {s.session_type === 'TRAINING' ? 'Allenamento' : 'Partita'}</b><small>{s.title || 'Seduta'}{s.session_type === 'MATCH' ? ` · ${callMap.size} convocati` : ''}</small></div><div className="attendance-chips">{relevant.map(p => { const r = recMap.get(p.id); const c = callMap.get(p.id); const label = s.session_type === 'TRAINING' ? (r ? (r.status === 'PRESENT' ? 'Presente' : `Assente · ${reasonLabels[r.absence_reason] || 'Altro'}`) : 'Non registrato') : (c ? (c.status === 'CALLED_UP' ? 'Convocato' : (c.custom_status || reasonLabels[c.status] || c.status)) : 'Non convocato'); return <span className={`status ${label === 'Presente' || label === 'Convocato' ? 'good' : label === 'Non registrato' || label === 'Non convocato' ? '' : 'bad'}`} key={p.id}><b>{p.last_name}</b> · {label}</span>; })}</div></div> })}</div> : <Empty title="Nessuna seduta nel periodo" text="Crea sedute per popolare il registro cronologico."/>}
    </section>
    {tab === 'player' && player !== 'ALL' && <section><div className="section-bar"><div><h2>Dettaglio storico</h2><p>Allenamenti filtrati nel periodo selezionato.</p></div></div><div className="cards">{detail.map(r => <div className="info" key={r.id}><div><b>{sessions.find(s => s.id === r.session_id)?.session_date ? date(sessions.find(s => s.id === r.session_id).session_date) : '—'}</b><small>{sessions.find(s => s.id === r.session_id)?.session_type === 'TRAINING' ? 'Allenamento' : 'Partita'} · {r.status === 'PRESENT' ? 'Presente' : `Assente — ${reasonLabels[r.absence_reason] || 'Altro'}`}</small></div>{r.status === 'PRESENT' ? <CheckCircle2 /> : <XCircle />}</div>)}{!detail.length && <div className="panel">Nessun record nel periodo.</div>}</div></section>}
    {tab === 'player' && player === 'ALL' && <Empty title="Seleziona un giocatore" text="Usa il filtro qui sopra per aprire il dettaglio individuale."/>}
    {modal && <Modal title={editing ? 'Modifica assenza programmata' : 'Nuova assenza programmata'} close={() => setModal(false)}><form className="form" onSubmit={savePlanned}><label>Giocatore<select required value={form.player_id} onChange={e => setForm({ ...form, player_id: e.target.value })}><option value="">Seleziona giocatore</option>{players.filter(p => p.status === 'ACTIVE').map(p => <option key={p.id} value={p.id}>{p.first_name} {p.last_name}</option>)}</select></label><div className="form-grid"><label>Dal<input type="date" required value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })}/></label><label>Al<input type="date" required value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })}/></label></div><label>Motivo<input placeholder="Es. ferie, lavoro, viaggio…" value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })}/></label><label>Note<textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}/></label><button className="primary wide" disabled={busy}>{busy ? 'Salvataggio…' : 'Salva assenza programmata'}</button></form></Modal>}
