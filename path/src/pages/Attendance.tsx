...
export default function Attendance() {
  ...
  const [players, setPlayers] = useState<any[]>([]), [staff, setStaff] = useState<any[]>([]), [sessions, setSessions] = useState<any[]>([]), [records, setRecords] = useState<any[]>([]), [staffRecords, setStaffRecords] = useState<any[]>([]), [calls, setCalls] = useState<any[]>([]), [planned, setPlanned] = useState<ProgrammedAbsence[]>([]);
  ...
  const [tab, setTab] = useState<'overview'|'planned'|'register'|'player'|'staff'>('overview');
  const [staffSessionId,setStaffSessionId]=useState(''); const [staffDraft,setStaffDraft]=useState<Record<string,any>>({});

  async function load() {
    if (!seasonId) return;
    const [p, ts, s, a, m, pa] = await Promise.all([
      supabase.from('players').select('*').eq('season_id', seasonId).order('last_name'),
      supabase.from('technical_staff').select('*').eq('season_id', seasonId).eq('status','ACTIVE').order('last_name'),
    ...
    setPlayers(p.data || []); setStaff(ts?.data || []); setSessions(s.data || []); setRecords(a.data || []); setPlanned(pa.data || []);
    if ((s.data||[]).length) {
      const { data: sar, error: sae } = await supabase.from('staff_attendance').select('*').in('session_id',(s.data||[]).map((x:any)=>x.id));
      if(sae) toast.push(sae.message,'error'); else setStaffRecords(sar||[]);
    } else setStaffRecords([]);
  ...
  async function openStaffSession(id:string){
    setStaffSessionId(id);
    if(!id){setStaffDraft({});return}
    const {data,error}=await supabase.from('staff_attendance').select('*').eq('session_id',id);
    if(error){toast.push(error.message,'error');return}
    setStaffDraft(Object.fromEntries(staff.map((s:any)=>[s.id,(data||[]).find((r:any)=>r.staff_id===s.id)||{status:'PRESENT',absence_reason:null}])));
  }
  async function saveStaffAttendance(){
    if(!staffSessionId)return;
    const rows=staff.map((s:any)=>({session_id:staffSessionId,staff_id:s.id,status:staffDraft[s.id]?.status||'PRESENT',absence_reason:staffDraft[s.id]?.status==='PRESENT'?null:(staffDraft[s.id]?.absence_reason||'OTHER')}));
    setBusy(true); const{error}=await supabase.from('staff_attendance').upsert(rows,{onConflict:'session_id,staff_id'});
    if(error)toast.push(error.message,'error');else{toast.push('Presenze staff salvate');await load()}
    setBusy(false);
  }
  ...
  return (
    <Page season={season} title="Presenze" sub="Storico presenze e promemoria delle indisponibilità programmate"
      ...
      <SectionTabs value={tab} onChange={x => setTab(x as any)} tabs={[
        ...
        { id: 'player', label: 'Dettaglio giocatore' },{ id: 'staff', label: 'Staff tecnico', count: staff.length }
      ]} />
      ...
      {tab === 'staff' && (
        <section>
          <div className="section-bar"><div><h2>Presenze staff tecnico</h2><p>Seleziona una seduta e registra separatamente la presenza dello staff.</p></div></div>
          <div className="filters"><select value={staffSessionId} onChange={e=>openStaffSession(e.target.value)}>
            <option value="">Seleziona una seduta</option>
            {sessions.map((s:any)=><option key={s.id} value={s.id}>{date(s.session_date)} · {s.session_type==='TRAINING'?'Allenamento':'Partita'} · {s.title||'Seduta'}</option>)}
          </select></div>
          {staffSessionId&&<><div className="cards">{staff.map((s:any)=>{const r=staffDraft[s.id]||{status:'PRESENT'};return <div className="info" key={s.id}>
            <div><b>{s.first_name} {s.last_name}</b><small>{({HEAD_COACH:'Allenatore',ASSISTANT_COACH:'Vice allenatore',FITNESS_COACH:'Preparatore atletico',GOALKEEPER_COACH:'Preparatore portieri',TECHNICAL_COLLABORATOR:'Collaboratore tecnico',TEAM_MANAGER:'Dirigente accompagnatore',OTHER:'Altro'} as any)[s.role]||'Staff'}</small></div>
            <div className="attendance-main-choice"><button type="button" className={r.status!=='ABSENT'?'small active-choice':'small'} onClick={()=>setStaffDraft(x=>({...x,[s.id]:{...r,status:'PRESENT',absence_reason:null}}))}>Presente</button><button type="button" className={r.status==='ABSENT'?'small active-choice':'small'} onClick={()=>setStaffDraft(x=>({...x,[s.id]:{...r,status:'ABSENT',absence_reason:r.absence_reason||'OTHER'}}))}>Assente</button>{r.status==='ABSENT'&&<div className="absence-quick">{attendanceOptions.map(([v,l])=><button type="button" key={v} className={r.absence_reason===v?'small active-choice':'small'} onClick={()=>setStaffDraft(x=>({...x,[s.id]:{...r,status:'ABSENT',absence_reason:v}}))}>{l}</button>)}</div>}</div>
          </div>})}</div><button className="primary wide" onClick={saveStaffAttendance} disabled={busy}>{busy?'Salvataggio…':'Salva presenze staff'}</button></>}
          {!staffSessionId&&<Empty title="Seleziona una seduta" text="Qui puoi registrare le presenze dello staff senza mischiarle con quelle dei giocatori."/>}
          {!staff.length&&<Empty title="Nessuno staff attivo" text="Aggiungi lo staff dalla sezione Rosa → Staff tecnico."/>}
        </section>
      )}
